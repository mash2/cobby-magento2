...
class UrlManagement extends AbstractManagement implements \Mash2\Cobby\Api\ImportProductUrlManagementInterface
{
    ...
    protected function categoriesUrlRewriteGenerate()
    {
        ...
        $result = $this->permanentCategoriesRewriteGenerate($urls);

        return $urls + $result;
    }

    protected function permanentCategoriesRewriteGenerate($urls)
    {
        $index = count($urls);
        $result = [];
        foreach ($this->products as $productsByStores) {
            foreach ($productsByStores as $product) {
                foreach ($urls as $url) {
                    if( $product->getEntityId() == $url->getEntityId() ){
                        $result[$index] = $this->urlRewriteFactory->create()
                            ->setEntityType($url->getEntityType())
                            ->setEntityId($url->getEntityId())
                            ->setTargetPath($url->getRequestPath())
                            ->setRequestPath(str_replace($product->getUrlKey(), $product->getOldUrlKey(), $url->getRequestPath()))
                            ->setRedirectType(OptionProvider::PERMANENT)
                            ->setDescription(null)
                            ->setIsAutogenerated(1)
                            ->setStoreId($url->getStoreId())
